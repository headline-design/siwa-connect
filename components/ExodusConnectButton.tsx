...
const ExodusConnectButton = ({
  ...
  const handleClick = async () => {
    try {
      ...
      console.error("Error connecting to Exodus:", error);
    }
  };
  return (
    <button
      ...
      onClick={handleClick}
    >
      {isLoading ? <LoadingSpinner /> : <><IconPerra className="mr-2"/> Connect Exodus</>}
    </button>
  );
};

export default ExodusConnectButton;
